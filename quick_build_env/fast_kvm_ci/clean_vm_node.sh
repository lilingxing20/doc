...
echo ${NODE_ARRAY[$idx]} | while read  VM_NAME HOST_NAME CPU MEM VM_DISKS VM_NICS VM_NETS PASSWORD ROLE; do
    [ $ROLE == 'bm' ] && vbmc delete "$VM_NAME"
    virsh list --all | grep $VM_NAME && virsh destroy $VM_NAME
    virsh list --all | grep $VM_NAME && virsh undefine $VM_NAME --nvram
    tmp_vm_dir="$TMP_DIR/${VM_NAME}"
    tmp_vm_eth_dir="${tmp_vm_dir}/eth"
    tmp_vm_hn_file="${tmp_vm_dir}/hostname"
    test -d $tmp_vm_eth_dir && rm -rfv $tmp_vm_eth_dir
    test -f $tmp_vm_hn_file && rm -fv $tmp_vm_hn_file
