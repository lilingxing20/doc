...
function set_vm_hostname() {
    vm_boot_disk=$1
    vm_host_name=$2
    virt-sysprep -a ${vm_boot_disk} --hostname ${vm_host_name} --selinux-relabel  --quiet
}
...
function set_vm_eth_cfg() {
    ...
    do
      virt-sysprep -a ${vm_boot_disk} --upload $eth_cfg_dir/$eth_file:/etc/sysconfig/network-scripts/$eth_file --quiet
    done
}
...
function set_vm_etc_hosts() {
    ...
    if [ -f "$etc_hosts_file" ];then
        virt-sysprep -a ${vm_boot_disk} --upload $etc_hosts_file:/etc/hosts --quiet
    fi
}
...
function set_vm_time_zone() {
    ...
    virt-sysprep -a ${vm_boot_disk} --timezone ${time_zone} --quiet
}
...
function set_vm_root_pwd() {
    ...
    if [ -n "$vm_root_pwd" ];then
        virt-sysprep -a ${vm_boot_disk} --root-password password:${vm_root_pwd} --quiet
    fi
}
...
function set_vm_firstboot_script() {
    ...
    do
        virt-sysprep -a ${vm_boot_disk} --firstboot "${firstboot_script_dir}/${script_file}" --quiet
    done
}

function set_vm_ssh_cfg() {
    vm_boot_disk=$1
    virt-sysprep -a ${vm_boot_disk} --ssh-inject root --edit '/etc/ssh/sshd_config:s/GSS/#GSS/' --edit '/etc/ssh/sshd_config:s/#UseDNS yes/UseDNS no/' --quiet
}
...
function make_vm_domain() {
    ...
    for network_name in $(echo $vm_nets | awk -F";" '{for(i=1;i<=NF;i++) print $i}')
    do
        cmd="$cmd --network network=${network_name},model=virtio"
    done
    ${cmd}
}
...
function attach_volumes() {
    ...
    echo "$disks_size" | awk -F";" '{for(i=1;i<=NF;i++) print i" "$i}' | while read idx disk_size
    do
        [ -z "$disk_size" ] && break
